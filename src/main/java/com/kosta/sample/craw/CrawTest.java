...
public class CrawTest {
	...
	public ArrayList<NewsVO> getNaverNews() {
		...
		Elements liList = doc.select("#newsct > div.section_latest > div > div.section_latest_article._CONTENT_LIST._PERSIST_META > div:nth-child(1) > ul > li"); //반복되는 요소를 찾는다
		//System.out.println(el.toString());
		
		ArrayList<NewsVO> list = new ArrayList<NewsVO>();
		for(Element li :liList) {
			String href = li.select("div > div > div.sa_thumb._LAZY_LOADING_ERROR_HIDE > div > a").attr("href");
			//Element e = li.select("div > div > div.sa_thumb._LAZY_LOADING_ERROR_HIDE > div > a > img")
